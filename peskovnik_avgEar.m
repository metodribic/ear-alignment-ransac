...
function peskovnik_avgEar()
    ...
    leftW = 83;
    rightW = 80;
    avg_left = zeros(100, leftW);
    avg_right = zeros(100, rightW);
        ...
        for j = 1:10
            ...
            if strcmp(current_data.d, side) == 1
                ...
                if current_data.accessories == 0 && current_data.overlap == 0 && current_data.hYaw == 0 && ...
                    ...
                    % ?e je prvi ga samo priredi?
                    if counter == 0
                        if strcmp(current_data.d, 'l')
                            l = floor((leftW - w)/2);
                            d = round((leftW - w)/2);
                            avg_left = [zeros(100,l) image zeros(100,d)];
                        else
                            l = floor((rightW-w)/2);
                            d = round((rightW-w)/2);
                            avg_right = [zeros(100,l) image zeros(100,d)];
                        end
                    ...
                    % ?e ne jih se?teva?
                        ...
                        if strcmp(current_data.d, 'l')
                            l = floor((leftW - w)/2);
                            d = round((leftW - w)/2);
                            avg_left = avg_left + [zeros(100,l) image zeros(100,d)];
                        % desn    
                        else
                            l = floor((rightW-w)/2);
                            d = round((rightW-w)/2);
                            avg_right = avg_right + [zeros(100,l) image zeros(100,d)];
                        end
%                         avg_image = avg_image + image;
    ...
    if strcmp(side, 'l')
        test_image = double(avg_left)./counter;
    else
        test_image = double(avg_right)./counter;
    end
