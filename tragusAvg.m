% find average ear new tragus
% THIS IS NOT THE SCRIPT FOR GETTING AVERAGE EAR - WHERE IS IT?
        ...
        for j = 1:10
            ...
            if strcmp(current_data.d, side) == 1
                ...
                if current_data.accessories == 0 && current_data.overlap == 0 && current_data.hYaw == 0 && ...
                    ...
                    % calculate new tragus
                    factor = x/100;
                    
                    if counter == 0
%                         avg_image = image;
                        tragus = struct();
                        tragus.x = current_data.x/factor;
                        tragus.y = current_data.y/factor;
                        counter = counter +1;
                    else
%                         avg_image = avg_image + image;
                        tragus.x = tragus.x + (current_data.x/factor);
                        tragus.y = tragus.y + (current_data.y/factor);
                        counter = counter +1;
                    end
                end
            end
...
%     test_image = ceil(test_image);
    tragus.x = tragus.x / counter;
    tragus.y = tragus.y / counter;
    
    figure(1); clf;
    imshow(uint8(test_image)); hold on
    plot(tragus.x, tragus.y,'r.','MarkerSize',30)
    disp(counter);
